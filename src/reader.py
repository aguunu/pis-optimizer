...
def load_problem(problem_path: str):
    ...
    slot_size = 30
    min_time_slot = 0
    max_time_slot = int((60 / slot_size) * 24 * days) # slots de 30 minutos
    
    # integrantes.csv
    # id, nombre
    df_integrantes = pd.read_csv(os.path.join(problem_path, "integrantes.csv"), header=None)
    ...
    duration = df_tareas[1].apply(lambda x: hhmm_to_slot(x, slot_size)).to_numpy(dtype=np.int64)
    task_types = df_tareas[2].to_numpy()
    task_demand = df_tareas[3].to_numpy()

    # id_worker, id_tipo, preferencia
    dict_task_preferences = {}
    df_preferences = pd.read_csv(os.path.join(problem_path, "preferencias_tareas.csv"), header=None)
    ...
    for w in range(total_members):
        for t in range(total_tasks):
            key = (w, task_types[t])
            preferences[w][t] = dict_task_preferences[key] if key in dict_task_preferences.keys() else 0 
            
    df_group_preferences = pd.read_csv(os.path.join(problem_path, "preferencias_grupos.csv"), header=None)
    ...
    df_dependencies = pd.read_csv(os.path.join(problem_path, "dependencias.csv"), header=None)
    ...
    df_restrictions = pd.read_csv(os.path.join(problem_path, "restricciones_horarias.csv"), header=None)
    ...
    for w, start, end in df_restrictions.itertuples(index=False):
        for i in range(days):
            ...
            interval = (int(day_offset + hhmm_to_slot(start, slot_size)), int(day_offset + hhmm_to_slot(end, slot_size)))
